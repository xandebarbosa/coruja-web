...
function getConfig(concessionaria?: string): ConcessionariaConfig {
  ...
  return (
    CONCESSIONARIA_CONFIG[concessionaria.toLowerCase().trim()] ?? DEFAULT_CONFIG
  );
}
...
function injectStyles() {
  if (
    ...
    document.getElementById("radar-map-styles")
  )
    return;
...
export default function RadarMapComponent({
    ...
    (point: RadarLocationDTO) => {
      ...
      if (!iconCache.current.has(key)) {
        ...
        iconCache.current.set(
          key,
          createCustomIcon(cfg.color, cfg.abbr, isSelected, cfg.logo),
        );
      }
  ...
  const filteredPoints = useMemo(() => {
    ...
    return safePoints.filter((p) => {
      ...
      return (
        !activeFilters || activeFilters.size === 0 || activeFilters.has(key)
      );
    });
  ...
  return (
    <>
      ...
      >
        ...
        />
        <span
          style={{ color: TOKENS.textMuted, fontSize: 11, fontWeight: 500 }}
        >
          RADARES
        </span>
        <span
