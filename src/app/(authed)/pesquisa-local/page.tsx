...
export default function ConsultaLocal() {
  ...
  useEffect(() => {
    const fetchKms = async () => {
      ...
      try {
        ...
        const kmsData = await radarsService.getKmsByRodoviaId(
          Number(filters.rodoviaId),
  ...
  const fetchRadars = useCallback(
    async (page: number, size: number, filtersToUse: FilterState) => {
      setLoading(true);
      ...
      try {
        ...
        if (response?.content && Array.isArray(response.content)) {
          ...
          if (response.content.length === 0) {
            ...
            toast.success(
              `${response.page?.totalElements || response.content.length} registros encontrados.`,
            );
          }
        } else {
  ...
  const handleSearchClick = () => {
    ...
    if (!hasAdditionalFilter) {
      toast.warning(
        "⚠️ Selecione pelo menos um filtro adicional além da data.",
      );
      return;
    }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (data.length > 0) {
        exportToExcel(
          data,
          `Relatorio_Radares_${filtersToExport.concessionaria}`,
        );
